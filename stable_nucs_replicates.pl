...
###==================================================================================================
### Finds stable nucleosomes using all replicates for the same cell state
...
my $usage = "$0 -input=\"path to working dir\" -output=\"path to results file\" -coordsCol=0 -occupCol=2 -chr=\"chr1\" -StableThreshold=0.5 -printData\n";
...
if (@ARGV != 0) {
	...
	if ($comand_line_flag =~ /-chr=(.*)/i) { $chr = $1; }
	if ($comand_line_flag =~ /-StableThreshold=(.*)/i) { $StableThreshold= $1; }
...
for my $position ( sort {$a<=>$b} keys %occupancy) {
    ...
    if (($rel_err < $StableThreshold) && ($rel_err!=0)) { 
    	if ($addData eq "yes") { print OUT join("\t",$chr, $position-100, $position,$Mean,$stdev,$rel_err,@temp),"\n";   }
    	else {print OUT join("\t",$chr, $position-100, $position,$Mean,$stdev,$rel_err,),"\n";   }
    }
    undef @temp;
}
print STDERR "done!\n";
